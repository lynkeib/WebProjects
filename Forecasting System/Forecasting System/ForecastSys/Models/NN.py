...
class NN(object):
    ...
    def __init__(self, path_df, date):
        ...
        # self.temp = pd.concat(
        #     [DateTime, self.data.iloc[:, 2], self.data.loc[:, self.data.columns.str.contains("Load")]], axis=1)
        self.temp = pd.concat(
            [DateTime, self.data.iloc[:, 2]], axis=1)
    ...
    def predict_model_select(self):
        ...
        x_train = self.temp['2014-01-01 07:00':str(self.train_end_date)]['Load'].tolist()
        self.dataset = windowed_dataset(x_train, self.window_size, self.batch_size, self.shuffle_buffer_size)
    ...
    def model_selection_mape_rmse(self):
        ...
        series = np.array(self.temp[str(self.test_start_date):str(self.test_end_date)]['Load'].tolist())
        ...
        for counter in range(self.training_days):
            ...
            series = np.array(self.temp[str(X_start):str(Y_end)]['Load'].tolist())
    ...
    def predict_next_40hours(self):
        ...
        X_end = today
        X = np.array(self.temp[str(X_start):str(X_end)]['Load'].tolist())
