...
class DR(object):
    ...
    def model_building(self, x_train, station='RIV'):
        ml = sm.ols(formula=station + "_Temp_Log~Load_Lag_48+Humi_Lag_48+I(Load_Lag_48**2)+I(Humi_Lag_48**2)+\
                                   Hour+Weekday+Month+Holiday+ RIV_Temp_Log_Lag_48+I(RIV_Temp_Log_Lag_48**2)+\
                                       ...
                                       Holiday:Load_Lag_48+Holiday:Humi_Lag_48", data=x_train).fit()
        return ml
    ...
    def model_selection_mape_rmse(self):
        ...
        for counter in range(self.training_days):
            ...
            x_test.append(np.array(test['RIV_Temp']))

            # print(p)

            # print('mape', mape(test['RIV_Temp'], np.exp(p[0])))
            # t = np.exp(p[0])
    ...
    def predict_next_40hours(self, station='RIV'):
        today = self.datetime
